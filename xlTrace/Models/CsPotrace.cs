...
namespace CsPotrace {
  ...
  public partial class Potrace {
    ...
    static ArrayList GetMonotonIntervals(IntPoint[] Pts) {
      ...
      if (L.Count / 2 * 2 != L.Count) {// Connect the Last with first 
    ...
    static void Xor_Path(bool[,] Matrix, Path P) {
      ...
      while (CurrentIntervals.Count > 0) {   // invertLine
        for (int k = 0; k < CurrentIntervals.Count - 1; k++) {
          int x1 = P.pt[((MonotonInterval)CurrentIntervals[k]).CurrentID].X + 1;
          int x2 = P.pt[((MonotonInterval)CurrentIntervals[k + 1]).CurrentID].X;
...
<g>", Width, Height);
    ...
    static Func<Potrace.DoublePoint, double, Potrace.DoublePoint> roundDoublePoint = (dp, th) => {
      if (th == 0) {
        return dp;
      }
      var pre = $"{th}".IndexOf(".");
      var k = pre > -1 ? Math.Pow(10, pre) : 1;
      var mx = ((dp.X * k) % (th * k)) / k;
      var my = ((dp.Y * k) % (th * k)) / k;
      return new Potrace.DoublePoint(
        dp.X + ((mx < th) ? -mx : mx),
        dp.Y + ((my < th) ? -my : my)
      );
    };
    public static dynamic ListOfCurveArray;
    public static dynamic Trace(Bitmap bmp, int threshold, double pitch) {
      ListOfCurveArray = new ArrayList();
      Potrace.curveoptimizing = true;
      var matrix = Potrace.BitMapToBinary(bmp, threshold);
      Potrace.potrace_trace(matrix, ListOfCurveArray);
      Parallel.For(0, ((ArrayList)ListOfCurveArray).Count, i => {
        //for (var i = 0; i < ListOfCurveArray.Count; i++) {
        var tops = (ArrayList)ListOfCurveArray[i];
        //Parallel.For(0,tops.Count, j => {
        for (var j = 0; j < tops.Count; j++) {
          var curves = (Potrace.Curve[])tops[j];
          for (var k = 0; k < curves.Length; k++) {
            var curve = (Potrace.Curve)curves[k];
            ListOfCurveArray[i][j][k] = new Potrace.Curve() {
              A = roundDoublePoint(curve.A, pitch),
              B = roundDoublePoint(curve.B, pitch),
              ControlPointA = roundDoublePoint(curve.ControlPointA, pitch),
              ControlPointB = roundDoublePoint(curve.ControlPointB, pitch),
            };
          }
        }//);
      });
      return ListOfCurveArray;
    }
    public static string TracedSvgString = "";
    public static string TraceSvg(Bitmap bmp, int threshold, double pitch) {
      //return TracedSvgString = Potrace.Export2SVG(Trace(bmp, threshold, pitch), bmp.Width, bmp.Height);
      return TracedSvgString = Potrace.Export2SVG(ListOfCurveArray, bmp.Width, bmp.Height);
    }
    public static Bitmap TracedBmp = new Bitmap(1,1); 
    public static Bitmap TraceBmp(Bitmap bmp, int threshold, double pitch) {
      return TracedBmp = Potrace.Export2GDIPlus(ListOfCurveArray, bmp.Width, bmp.Height);
    }
    public static Func<string, List<List<Vector3>>> TraceFaces = (svg) => {
      var rgxSvgML = new Regex(@"[ML](\d+\.\d+) (\d+\.\d+)");
      var m = rgxSvgML.Matches(svg);
      var faces = new List<List<Vector3>>();
      var iEnd = m.Count;
      for (var i = 0; i < m.Count; i++) {
        var face = new List<Vector3>();
        var isM = m[i].Value[0] == 'M';
        face.Add(new Vector3(float.Parse(m[i].Groups[1].Value), float.Parse(m[i].Groups[2].Value), 0));
        var j = i;
        while (++j < m.Count && m[j].Value[0] == 'L') {
          face.Add(new Vector3(float.Parse(m[j].Groups[1].Value), float.Parse(m[j].Groups[2].Value), 0));
        }
        faces.Add(face);
        if (j == m.Count) { break; }
        i=j-1;
